...
<dialog class="moment-lightbox">
	<a href="<?= $page->parent()->url() ?>" class="moment-close" aria-label="<?= t('femundfilou.kirby-moments.aria-label.close') ?>">
...
?>
		<div class="moment-controls">
			...
			<?php if ($page->hasNext($allMoments)) : ?>
				<a href="<?= $page->next($allMoments)->url() ?>" class="moment-controls__next" aria-label="<?= t('femundfilou.kirby-moments.aria-label.next') ?>">
