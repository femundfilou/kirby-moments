...
<dialog class="moment-lightbox">
	<a href="<?= $page->parent()->url() ?>" class="moment-close" aria-label="<?= t('femundfilou.moments.aria-label.close') ?>">
...
?>
		<div class="moment-controls">
			<?php if ($page->hasPrev($allMoments)) : ?>
				<a href="<?= $page->prev($allMoments)->url() ?>" class="moment-controls__prev" aria-label="<?= t('femundfilou.moments.aria-label.prev') ?>">
					<?php snippet('moments-icon/prev'); ?>
				</a>
			<?php endif; ?>
			<?php if ($page->hasNext($allMoments)) : ?>
				<a href="<?= $page->next($allMoments)->url() ?>" class="moment-controls__next" aria-label="<?= t('femundfilou.moments.aria-label.next') ?>">
